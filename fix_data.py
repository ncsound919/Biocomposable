...
# Extract layers
layers_match = re.search(r'export const layers: Layer\[\] = \[.*?\];', text, re.DOTALL)
layers_str = layers_match.group(0) if layers_match else ""

# Extract contracts
contracts_match = re.search(r'export const contracts: Contract\[\] = \[.*?\];', text, re.DOTALL)
contracts_str = contracts_match.group(0) if contracts_match else ""

# Extract determinismData
det_match = re.search(r'export const determinismData: DeterminismItem\[\] = \[.*?\];', text, re.DOTALL)
det_str = det_match.group(0) if det_match else ""
